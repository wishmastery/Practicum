...
double neuralNetwork::getSetAccuracy( std::vector<dataEntry*>& set )
{
    ...
    vector<vector<double>> gsetresult;
    ...
    for ( int tp = 0; tp < (int) set.size(); tp++)
    {
        ...
        // log generalization set result
        vector<double> temp;
        gsetresult.push_back(temp);
        gsetresult[tp].push_back(outputNeurons[0]);
        // gsetresult[tp].push_back(outputNeurons[1]);
        // gsetresult[tp].push_back(outputNeurons[2]);
    ...
    if (!loggset.is_open())
    {
        loggset.open("g_set_result.csv", ios::out);
        
        if ( loggset.is_open() )
        {
            //write log file header
            for(int i = 0; i < gsetresult.size(); i++){
                loggset << gsetresult[i][0]
                // << "," << gsetresult[i][1] << "," << gsetresult[i][2]
                << endl;
            }
        }
    }
...
/*******************************************************************
 * Return the NN validation set accuracy on the set
 ********************************************************************/
double neuralNetwork::getVSetAccuracy( std::vector<dataEntry*>& set )
{
    double incorrectResults = 0;
    vector<vector<double>> vsetresult;
    ...
    for ( int tp = 0; tp < (int) set.size(); tp++)
    {
        ...
        // log validation set result
        vector<double> temp;
        vsetresult.push_back(temp);
        vsetresult[tp].push_back(outputNeurons[0]);
        // vsetresult[tp].push_back(outputNeurons[1]);
        // vsetresult[tp].push_back(outputNeurons[2]);
    ...
    if (!logvset.is_open())
    {
        logvset.open("v_set_result.csv", ios::out);
        
        if ( logvset.is_open() )
        {
            //write log file header
            for(int i = 0; i < vsetresult.size(); i++){
                logvset << vsetresult[i][0]
                // << "," << vsetresult[i][1] << ","  << vsetresult[i][2]
                << endl;
            }
        }
    }
